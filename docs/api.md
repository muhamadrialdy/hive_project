...
Last 10 rows, newest-first. Same row shape as `/data/table` → `records[]`.
...
MLOps routes. The trained model is `RandomForestRegressor(n_estimators=200, random_state=42)` over the 10 features described in [Architecture → Forecast](/architecture#forecast-get-apiforecastenterprisersdays7).

### `GET /ml/metrics`

Train on the full dataset minus the last 30 days, then evaluate on those 30 days. Returns MAE / RMSE / MAPE plus a status flag (`Healthy` if MAE < 50, else `Drifting`).
...
### `GET /forecast/enterprisers`

Iterative N-day forecast of `new_enterpriser_count`. Each predicted value is appended to the rolling window before the next prediction.
...
### `GET /notebook/files/content`

Read a file's text content. Paths must resolve inside `notebooks/`.

**Query params:** `path` (e.g. `notebooks/exploration.ipynb`).
